...
export function InfiniteCanvas({ isSorted = false }: InfiniteCanvasProps) {
  ...
  useEffect(() => {
      ...
      if (isSorted) {
          ...
          // If overlaps were resolved, we might want to save the new positions immediately?
          // The prompt says "application should automatically move others around it... especially when we add new memories".
          // "New position should not be retained" applied to SORT mode.
          // For default mode, if we resolve overlaps, should we save? 
          // Usually yes, to persist the clean state.
          // But let's be careful not to save on every render loop.
          // For now, just visual fix. If user drags, it saves.
          setNodes(defaultNodes);
      }
  }, [isSorted, rawMemories, setNodes]);
  
  // Effect to handle collision resolution whenever nodes change (e.g. drag end)
  // BUT be careful of infinite loops with useNodesState.
  // We only want to run this when node interaction ends.
  // We'll hook into onNodeDragStop for manual moves.
  // For initial load, createNodesFromMemories handles it.
